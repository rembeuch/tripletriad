    ...
    super do |resource|
      if resource.errors.empty?
        if @player.elites.empty? && @player.cards.empty? && @player.pnjs.empty?
          ...
          @Kosmos = Pnj.new(player: @player, dialogue: DialoguesManager::CASE_DIALOGUES[:A0][:welcome], zone_image: ZoneImagesManager::CASE_IMAGES[:A0], name: "Kosmos")
          if @Kosmos.save
            PnjObjectivesManager.create_objectives(@Kosmos)
          end
          @Aries = Pnj.new(player: @player, zone: 'A1', dialogue: DialoguesManager::CASE_DIALOGUES[:A1][:welcome], zone_image: ZoneImagesManager::CASE_IMAGES[:A1], name: "Aries")
          if @Aries.save
            PnjObjectivesManager.create_objectives(@Aries)
          end
        end
      end
    end
